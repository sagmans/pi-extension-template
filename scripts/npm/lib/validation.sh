...
readonly NPM_ACCOUNT_PATTERN='^[a-z0-9][a-z0-9-]{0,213}$'
...
readonly REVIEWER_PATTERN='^[A-Za-z0-9]([A-Za-z0-9-]{0,37}[A-Za-z0-9])?$'
readonly TAG_PATTERN_PATTERN='^v[A-Za-z0-9._/*-]*$'
readonly MAXIMUM_TAG_PATTERN_LENGTH=64
...
validate_npm_account() {
	local value="${1:-}"
	[[ "${value}" =~ ${NPM_ACCOUNT_PATTERN} ]] || fail 'NPM_ACCOUNT must be a lowercase npm account name'
}
...
validate_reviewer() {
	...
	[[ "${value}" != *--* ]] || fail 'REVIEWER must be a GitHub login'
}

validate_tag_pattern() {
	local value="${1:-}"
	[[ "${value}" =~ ${TAG_PATTERN_PATTERN} ]] || fail 'TAG_PATTERN contains unsupported characters'
	[ "${#value}" -le "${MAXIMUM_TAG_PATTERN_LENGTH}" ] || fail 'TAG_PATTERN is too long'
	case "${value}" in

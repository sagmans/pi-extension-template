...
if "${NPM_BIN}" view "${PKG_NAME}@${PKG_VERSION}" name version repository dist --json --registry "${NPM_REGISTRY}" >"${metadata_file}" 2>/dev/null; then
	:
else
	status=$?
...
if "${NPM_BIN}" access get status "${PKG_NAME}" --json --registry "${NPM_REGISTRY}" >"${access_file}" 2>/dev/null; then
	:
else
	status=$?
...
if "${NPM_BIN}" trust list "${PKG_NAME}" --json --registry "${NPM_REGISTRY}" >"${trust_file}" 2>/dev/null; then
	:
else
	status=$?
